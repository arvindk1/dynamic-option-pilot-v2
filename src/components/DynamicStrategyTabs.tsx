...
interface TradeOpportunity {
  ...
  score?: number;
  /** Added for filters that reference IV rank */
...
// ===================== All Opportunities (grouped by strategy) =====================
...
const AllOpportunitiesView: React.FC<AllOpportunitiesViewProps> = React.memo(
  ({
    strategies,
    ...
    const handleTradeExecution = async (opportunity: TradeOpportunity) => {
      ...
      try {
        ...
        await paperTradingService.executeTrade(opportunity, 1);

        toast({
          title: "Trade Executed Successfully",
          description: `${opportunity.symbol} ${opportunity.strategy_type} executed for $${opportunity.premium.toFixed(
            2
          )}`,
          duration: 5000,
        });
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-xs text-muted-foreground">
                          Last scan:{" "}
                          {strategy.last_updated ? new Date(strategy.last_updated).toLocaleTimeString() : "Never"}
                        </div>
                        <Button
                          size="sm"
                          variant="outline"
...
const StrategyOpportunitiesView: React.FC<StrategyOpportunitiesViewProps> = React.memo(
  ({ opportunities, loading, onTradeExecuted, onRefresh, isRefreshing }) => {
    const [executingTrades, setExecutingTrades] = useState<Set<string>>(new Set());
    const { toast } = useToast();

    // Deduplicate and memoize opportunities by ID to prevent React key warnings
    ...
    // Memoize trade execution handler to prevent re-renders
    const handleExecute = useCallback(
      ...
      [executingTrades, toast, onTradeExecuted]
    );
    ...
    if (uniqueOpportunities.length === 0) {
      return (
        <EmptyStateCard
          ...
          description="No opportunities match your current strategy and market conditions. Try adjusting your parameters or refreshing the scan."
          suggestions={[
            "Widen the DTE range (try 7-60 days)",
            "Try a different strategy category",
            "Lower minimum win rate threshold",
            "Scan more symbols in universe",
            "Check if markets are open",
          ]}
          onReset={() => {
            toast({
              title: "Reset Parameters",
              description: "Reset to defaults functionality coming soon.",
              duration: 2000,
            });
          }}
          onRefresh={onRefresh}
    ...
    // Simple virtualization for large lists (>10 items)
    const shouldVirtualize = uniqueOpportunities.length > 10;
    const [visibleRange, setVisibleRange] = useState({
      start: 0,
      end: Math.min(20, uniqueOpportunities.length),
    });
    const visibleOpportunities = shouldVirtualize
      ? uniqueOpportunities.slice(visibleRange.start, visibleRange.end)
      : uniqueOpportunities;

    // Intersection Observer for progressive loading with proper cleanup
    ...
    const loadMoreRef = useCallback(
      (node: HTMLDivElement | null) => {
        ...
        observerRef.current = new IntersectionObserver(
          (entries) => {
            if (entries[0].isIntersecting && visibleRange.end < uniqueOpportunities.length) {
              setVisibleRange((prev) => ({
                start: prev.start,
                end: Math.min(prev.end + 10, uniqueOpportunities.length),
              }));
            }
          },
          { threshold: 0.1 }
      ...
      [shouldVirtualize, visibleRange.end, uniqueOpportunities.length]
    );
      ...
      <div className="space-y-4">
        ...
        {shouldVirtualize && visibleRange.end < uniqueOpportunities.length && (
          <div ref={loadMoreRef} className="flex items-center justify-center py-4 text-sm text-muted-foreground">
            Loading more opportunities... ({visibleRange.end}/{uniqueOpportunities.length})
          </div>
        )}
      </div>
    );
...
export const DynamicStrategyTabs: React.FC<DynamicStrategyTabsProps> = React.memo(
  ({ onTradeExecuted, symbol = "SPY" }) => {
    const { measureRender } = usePerformanceMonitoring("DynamicStrategyTabs");
    ...
    const handleRefresh = async () => {
      try {
        ...
        // Refresh all strategies (matches button label)
    ...
    const tradeTypeCategories = ["high_probability", "quick_scalps", "swing_trades", "volatility_plays"] as const;

    // Check which row type is active
    const isStrategyTab = activeTab === "all" || categories.includes(activeTab);
    const isTradeTypeTab = tradeTypeCategories.includes(activeTab as any);

    const getOpportunitiesByTradeType = (
      tradeType: (typeof tradeTypeCategories)[number],
        ...
        case "volatility_plays":
          return allOpportunities.filter(
            (opp) =>
              ...
              (typeof opp.volatility_rank === "number" && opp.volatility_rank > 50)
          );
        default:
          return [];
      }
    ...
    useEffect(() => {
      ...
      // include strategies & getStrategyOpportunities for correctness
    }, [symbol]);
    ...
    if (error) {
      return (
        <Alert>
          ...
          <AlertDescription>Failed to load trading strategies: {error}</AlertDescription>
        </Alert>
      );
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsList className="h-auto p-2 bg-muted/30">
            <div className="space-y-2 w-full">
              ...
              >
                <div
                  className={`text-xs font-medium px-2 py-1 min-w-fit ${
                  ...
                  {tradeTypeCategories.map((tradeType) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="capitalize">{tradeType.replace("_", " ")}</span>
                        <Badge variant="secondary" className="ml-1 text-xs">
          ...
          <TabsContent value="all" className="mt-6">
            <div className="mb-6 p-4 bg-blue-50 rounded-lg border">
              ...
              <p className="text-sm text-blue-600 mt-1">
                Complete analysis across all {strategies.length} trading strategies with {allOpportunities.length} total
                opportunities.
              </p>
            </div>
                ...
                <div className={`mb-6 p-5 rounded-lg border-2 ${getCategoryColor(category)}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      {getCategoryIcon(category)}
                      <div>
                        <h3 className="text-lg font-bold capitalize">{category.replace("_", " ")} Strategies</h3>
                  ...
                  <p className="text-sm opacity-90 leading-relaxed">{getCategoryDescription(category)}</p>
                </div>
          ...
          {(["high_probability", "quick_scalps", "swing_trades", "volatility_plays"] as const).map((tradeType) => {
            const tradeTypeOpportunities = getOpportunitiesByTradeType(tradeType, allOpportunities);

            const getTradeTypeDescription = (type: typeof tradeType) => {
              switch (type) {
                case "high_probability":
              ...
              <TabsContent key={tradeType} value={tradeType} className="mt-6">
                <div className={`mb-6 p-4 rounded-lg border ${getCategoryColor(tradeType)}`}>
                  <div className="flex items-center space-x-2 mb-2">
                    {getCategoryIcon(tradeType)}
                    <h3 className="font-semibold capitalize">{tradeType.replace("_", " ")}</h3>
                  </div>
                  <p className="text-sm opacity-80">{getTradeTypeDescription(tradeType)}</p>
                  <p className="text-sm opacity-80 mt-1">{tradeTypeOpportunities.length} opportunities available</p>
                </div>
