...
from core.interfaces.data_provider_interface import IDataProvider, DataProviderType, DataProviderAdapter
...
class EngineRegistry:
    ...
    async def _initialize_data_providers(self, data_config: Dict[str, Any]):
        ...
        if not self.plugin_registry:
            logger.warning("No plugin registry provided - data providers will not be available")
            return
            
        # Get available data provider plugins
        available_plugins = self.plugin_registry.get_plugins_by_type(PluginType.DATA_PROVIDER)
        ...
        for plugin_instance in available_plugins:
            try:
                ...
                if provider_type:
                    self._engines[EngineType.DATA_PROVIDER.value][provider_type.value] = adapter
                    logger.info(f"✅ Registered data provider: {provider_type.value} ({plugin_name})")
                else:
    ...
    def _map_plugin_to_provider_type(self, plugin_name: str) -> Optional[DataProviderType]:
    ...
    async def _initialize_other_engines(self, engines_config: Dict[str, Any]):
        ...
        self._engines[EngineType.RISK_MANAGEMENT.value]['standard'] = StandardRiskEngine()
    ...
    def _configure_defaults(self, config: Dict[str, Any]):
        ...
        if default_provider in self._engines[EngineType.DATA_PROVIDER.value]:
            ...
            if available:
                ...
                logger.warning(f"Default provider {default_provider} not available, using {available[0]}")
        ...
        logger.info(f"Default data provider: {self._default_providers.get(EngineType.DATA_PROVIDER.value)}")
        logger.info(f"Fallback providers: {self._fallback_providers.get(EngineType.DATA_PROVIDER.value, [])}")
    
    def get_data_provider(self, provider_type: Optional[DataProviderType] = None) -> Optional[IDataProvider]:
        """
    ...
    def get_engine(self, engine_type: EngineType, engine_name: Optional[str] = None) -> Optional[Any]:
        """
    ...
    def get_engines_dict(self, provider_type: Optional[DataProviderType] = None) -> Dict[str, Any]:
        """
    ...
    def get_provider_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'default_data_provider': self._default_providers.get(EngineType.DATA_PROVIDER.value),
            'fallback_providers': self._fallback_providers.get(EngineType.DATA_PROVIDER.value, [])
        }
        ...
        for provider_name, provider in data_providers.items():
            if hasattr(provider, 'get_cache_stats'):
                try:
                    ...
                    logger.warning(f"Failed to get cache stats from {provider_name}: {e}")
...
class StandardRiskEngine:
    ...
    def calculate_position_size(self, opportunity: Dict[str, Any], account_size: float) -> int:
