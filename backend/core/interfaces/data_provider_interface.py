...
class Quote:
    ...
    def __init__(self, symbol: str, price: float, volume: int, timestamp: datetime,
                 ...
                 change: float = 0.0, change_percent: float = 0.0, **kwargs):
        self.symbol = symbol
        self.price = price
        self.volume = volume
...
class OptionContract:
    ...
    def __init__(self, symbol: str, strike: float, expiration: date, option_type: str,
                 bid: float, ask: float, last: float, volume: int, open_interest: int,
                 delta: Optional[float] = None, gamma: Optional[float] = None,
                 theta: Optional[float] = None, vega: Optional[float] = None,
                 implied_volatility: Optional[float] = None, **kwargs):
        self.symbol = symbol
        self.strike = strike
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'symbol': self.symbol,
            'strike': self.strike,
            'expiration': self.expiration.isoformat() if isinstance(self.expiration, date) else self.expiration,
...
class OptionChain:
    ...
    def __init__(self, symbol: str, underlying_price: float, expiration: date,
...
class IDataProvider(ABC):
    ...
    async def get_option_chain_structured(self, symbol: str, expiration: date, **kwargs) -> OptionChain:
        """Get option chain as structured OptionChain object."""
        chain_data = await self.get_options_chain(symbol, expiration=expiration, **kwargs)
        ...
        for call_data in chain_data.get('calls', []):
            calls.append(OptionContract(
                ...
                open_interest=call_data.get('openInterest', call_data.get('open_interest', 0)),
                delta=call_data.get('delta'),
                gamma=call_data.get('gamma'),
                theta=call_data.get('theta'),
                vega=call_data.get('vega'),
                implied_volatility=call_data.get('impliedVolatility', call_data.get('implied_volatility'))
            ))
        ...
        for put_data in chain_data.get('puts', []):
            puts.append(OptionContract(
                ...
                open_interest=put_data.get('openInterest', put_data.get('open_interest', 0)),
                delta=put_data.get('delta'),
                gamma=put_data.get('gamma'),
                theta=put_data.get('theta'),
                vega=put_data.get('vega'),
                implied_volatility=put_data.get('impliedVolatility', put_data.get('implied_volatility'))
            ))
...
class DataProviderAdapter(IDataProvider):
    ...
    def __init__(self, provider_plugin):
        ...
        self._logger = logging.getLogger(f"{__name__}.{provider_plugin.__class__.__name__}")
