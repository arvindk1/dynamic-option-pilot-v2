        ...
        try:
            ...
            for required_field in self.metadata.config_schema.get('required', []):
                if required_field not in config:
                    self._logger.error(f"Missing required config field: {required_field}")
                    return False
    ...
    async def _safe_initialize(self) -> bool:
        ...
        try:
            ...
            if result:
                ...
                self._logger.error(f"Plugin initialization failed: {self.metadata.name}")
            
            return result
        except Exception as e:
            self.status = PluginStatus.ERROR
            self._logger.error(f"Plugin initialization error: {self.metadata.name} - {e}")
            return False
    ...
    async def _safe_cleanup(self) -> bool:
        ...
        try:
            ...
            if result:
                ...
                self._logger.info(f"Plugin cleaned up successfully: {self.metadata.name}")
            else:
...
class TradingStrategyPlugin(BasePlugin):
    ...
    @abstractmethod
    async def scan_opportunities(self, universe: List[str], **kwargs) -> List[Dict[str, Any]]:
...
class RiskManagementPlugin(BasePlugin):
    ...
    @abstractmethod
    async def assess_risk(self, portfolio: Dict[str, Any], trade: Dict[str, Any]) -> Dict[str, Any]:
