...
from core.strategies.json_strategy_loader import JSONStrategyLoader, JSONStrategyConfig
...
class StrategyRegistry:
    ...
    def __init__(self, plugin_registry: PluginRegistry, engine_registry: Optional[EngineRegistry] = None):
    ...
    def register_strategy_class(self, strategy_class: Type[BaseStrategyPlugin], config: StrategyConfig):
        """Register a strategy class with configuration."""
        strategy_id = config.strategy_id
        
        if strategy_id in self._strategies:
            self._logger.warning(f"Strategy {strategy_id} already registered, updating...")
    ...
    def initialize_json_strategy_loader(self, strategies_dir: str = None):
        ...
        if strategies_dir is None:
            ...
            raw_environment = os.getenv('TRADING_ENVIRONMENT', settings.trading.environment)
            environment = raw_environment
            
            self._logger.info(f"Raw environment from settings: {settings.trading.environment}")
            self._logger.info(f"TRADING_ENVIRONMENT env var: {os.getenv('TRADING_ENVIRONMENT', 'NOT SET')}")
            ...
            env_strategies_dir = project_root / "backend" / "config" / "strategies" / environment
            
            self._logger.info(f"Checking environment directory: {env_strategies_dir}")
            self._logger.info(f"Environment directory exists: {env_strategies_dir.exists()}")
            ...
            if env_strategies_dir.exists():
                ...
                self._logger.info(f"JSON files found in environment directory: {len(json_files)}")
                if json_files:
                    strategies_dir = str(env_strategies_dir)
                    self._logger.info(f"Using environment-specific strategies directory: {strategies_dir} ({len(json_files)} files)")
                else:
                    self._logger.warning(f"Environment directory {env_strategies_dir} exists but is empty")
            else:
                self._logger.warning(f"Environment directory {env_strategies_dir} does not exist")
            ...
            if strategies_dir is None:
                ...
                if rules_dir.exists():
                    ...
                    if json_files:
                        ...
                        self._logger.info(f"Falling back to rules directory: {strategies_dir} ({len(json_files)} files)")
                    else:
                        self._logger.error(f"Rules directory {rules_dir} exists but is empty")
                        
            # Final fallback - let JSONStrategyLoader use its default
            if strategies_dir is None:
                self._logger.warning("No strategy directories with JSON files found, using JSONStrategyLoader default")
        else:
            # Extract environment from provided path if possible
            environment = strategies_dir.split('/')[-1] if '/' in strategies_dir else 'custom'
        
        self._json_strategy_loader = JSONStrategyLoader(strategies_dir)
        actual_dir = self._json_strategy_loader.strategies_dir
        self._logger.info(f"Initialized JSON strategy loader with directory: {actual_dir}")
    ...
    def register_json_strategies(self) -> int:
        ...
        if not self.engine_registry:
            self._logger.error("Engine registry not available - cannot register JSON strategies")
            return 0
        
        json_strategies = self._json_strategy_loader.load_all_strategies(include_inactive=False)
        registered_count = 0
        ...
        for json_config in json_strategies:
            try:
                ...
                strategy_config = StrategyConfig(
                    ...
                    category=json_config.strategy_type.lower().replace('_', ' ').title(),
                    min_dte=json_config.position_parameters.get('min_dte', 7),
                    max_dte=json_config.position_parameters.get('max_dte', 45),
                    min_probability=json_config.position_parameters.get('min_probability', 0.65),
                    max_opportunities=json_config.position_parameters.get('max_opportunities', 10),
                    symbols=json_config.universe.get('preferred_symbols', ['SPY', 'QQQ', 'IWM']) if json_config.universe else ['SPY', 'QQQ', 'IWM'],
                    min_liquidity_score=json_config.position_parameters.get('min_liquidity_score', 7.0),
                    max_risk_per_trade=json_config.risk_management.get('max_loss_per_trade', 500.0)
                )
                ...
                registered_count += 1
                self._logger.info(f"Registered JSON strategy: {json_config.strategy_id} ({json_config.strategy_type})")
                
            except Exception as e:
                self._logger.error(f"Failed to register JSON strategy {json_config.strategy_id}: {e}")
        
        self._logger.info(f"Registered {registered_count} JSON strategies")
        return registered_count
        
    async def create_strategy_instance(self, strategy_id: str) -> Optional[BaseStrategyPlugin]:
        ...
        try:
            ...
            if registration.strategy_class == JSONStrategyPlugin:
                if not self._json_strategy_loader:
                    self._logger.error(f"JSON strategy loader not initialized for {strategy_id}")
                    return None
            ...
            self._logger.info(f"Created and initialized strategy instance: {strategy_id}")
    ...
    async def initialize_all_strategies(self) -> bool:
        ...
        self._logger.info(f"Initialized {success_count}/{total_count} strategies (success rate: {success_rate:.1%})")
    ...
    async def scan_all_strategies(self, universe: List[str], **kwargs) -> Dict[str, List[StrategyOpportunity]]:
        ...
        for strategy_id, instance in self._strategy_instances.items():
            registration = self._strategies[strategy_id]
            if registration.enabled:
                task = self._scan_strategy_safe(strategy_id, instance, universe, **kwargs)
                tasks.append(task)
        ...
        # Process results
        strategy_ids = [sid for sid, inst in self._strategy_instances.items() if self._strategies[sid].enabled]
    ...
    async def _scan_strategy_safe(self, strategy_id: str, instance: BaseStrategyPlugin, 
                                  universe: List[str], **kwargs) -> List[StrategyOpportunity]:
        """Safely scan a strategy with error handling."""
        try:
            opportunities = await instance.scan_opportunities(universe, **kwargs)
            self._logger.debug(f"Strategy {strategy_id} found {len(opportunities)} opportunities")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        for category, strategy_ids in self._categories.items():
            enabled_in_category = sum(1 for sid in strategy_ids if self._strategies[sid].enabled)
            categories_stats[category] = {
                'total': len(strategy_ids),
                'enabled': enabled_in_category,
                'active_instances': sum(1 for sid in strategy_ids if sid in self._strategy_instances)
            }
    ...
    async def cleanup_all(self) -> bool:
        ...
        if cleanup_tasks:
            ...
            self._logger.info(f"Cleaned up {success_count}/{len(cleanup_tasks)} strategy instances")
