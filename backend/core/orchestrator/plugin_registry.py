...
from .base_plugin import BasePlugin, PluginType, PluginStatus, PluginConfig
from .event_bus import EventBus, PluginEvent, EventType
...
class PluginRegistry:
    ...
    async def create_plugin(self, plugin_name: str, config: PluginConfig = None) -> BasePlugin:
        ...
        async with self._lock:
            ...
            await self._event_bus.emit(PluginEvent(
    ...
    def _resolve_initialization_order(self) -> List[str]:
        ...
        if len(order) != len(self._plugins):
            ...
            raise DependencyError(f"Circular dependency detected among plugins: {remaining}")
    ...
    async def initialize_all(self) -> bool:
        ...
        async with self._lock:
            try:
                ...
                logger.info(f"Plugin initialization order: {self._initialization_order}")
                ...
                total_enabled = sum(1 for p in self._plugins.values() if p.config.enabled)
                logger.info(f"Initialized {success_count}/{total_enabled} enabled plugins")
    ...
    def get_plugins_by_type(self, plugin_type: PluginType) -> List[BasePlugin]:
        ...
        return [p for p in self._plugins_by_type[plugin_type] 
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        active_plugins = sum(1 for p in self._plugins.values() 
