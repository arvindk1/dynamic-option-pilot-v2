...
class MarketHours:
    ...
    def is_peak_hours(self, dt: datetime = None) -> bool:
        ...
        afternoon_peak = self.peak_afternoon_start <= current_time <= self.peak_afternoon_end
...
class OptionsScheduler:
    ...
    def __init__(self, plugin_registry: PluginRegistry, opportunity_cache: OpportunityCache):
    ...
    async def _schedule_strategy_jobs(self):
        ...
        for strategy, config in self.strategy_configs.items():
            ...
            logger.info(f"Scheduled {strategy} scanning every {config['interval_minutes']} minutes")
    ...
    async def _schedule_maintenance_jobs(self):
        ...
        self.scheduler.add_job(
            ...
            trigger=CronTrigger(hour=9, minute=25, day_of_week="mon-fri"),  # 5 min before market open
    ...
    async def _scan_strategy(self, strategy: str):
        ...
        try:
            ...
            opportunities = await self._perform_strategy_scan(strategy, config, data_plugin)
            ...
            if opportunities:
                ...
                logger.info(f"{strategy} scan completed: {len(opportunities)} opportunities found")
            else:
            ...
            try:
                ...
                if session:
                    ...
                    session.opportunities_found = len(opportunities) if opportunities else 0
    ...
    async def _perform_strategy_scan(self, strategy: str, config: Dict[str, Any], 
                                   data_plugin) -> List[Dict[str, Any]]:
        """
    ...
    async def _database_maintenance(self):
        ...
        try:
            ...
            db = next(db_gen)
            try:
                old_sessions = db.query(ScanSession).filter(
                ...
                if old_sessions:
                    logger.info(f"Database maintenance: cleaned up {len(old_sessions)} old scan sessions")
            finally:
                db.close()
...
def initialize_options_scheduler(plugin_registry: PluginRegistry, 
                               opportunity_cache: OpportunityCache) -> OptionsScheduler:
