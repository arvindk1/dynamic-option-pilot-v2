...
class JSONStrategyLoader:
    ...
    def __init__(self, strategies_dir: str = None):
        if strategies_dir is None:
            ...
            strategies_dir = project_root / "backend" / "config" / "strategies" / "rules"
        ...
        self.required_fields = {
            'strategy_name', 'strategy_type', 'module', 
            'position_parameters', 'entry_signals', 'scoring'
        }
        
        logger.info(f"JSONStrategyLoader initialized with directory: {self.strategies_dir}")
    
    def load_all_strategies(self, include_inactive: bool = False) -> List[JSONStrategyConfig]:
        """
    ...
    def load_strategy(self, strategy_id: str, use_cache: bool = True) -> Optional[JSONStrategyConfig]:
        """
        ...
        if strategy_config:
            ...
            self._file_timestamps[strategy_id] = datetime.fromtimestamp(json_file.stat().st_mtime)
    ...
    def _load_strategy_file(self, json_file: Path) -> Optional[JSONStrategyConfig]:
        ...
        try:
            ...
            if not validation_result.is_valid:
                logger.error(f"Invalid strategy config {strategy_id}: {validation_result.errors}")
                return None
    ...
    def _validate_strategy_config(self, strategy_id: str, config: Dict[str, Any]) -> StrategyValidationResult:
        ...
        valid_types = {
            'THETA_HARVESTING', 'RSI_COUPON', 'IRON_CONDOR', 'PUT_SPREAD', 
            'COVERED_CALL', 'CREDIT_SPREAD', 'VERTICAL_SPREAD', 'STRADDLE',
            'STRANGLE', 'BUTTERFLY', 'CALENDAR_SPREAD', 'COLLAR',
            'PROTECTIVE_PUT', 'NAKED_OPTION'
        }
                ...
                if not isinstance(rules, list):
                    ...
                    for i, rule in enumerate(rules):
                        if not isinstance(rule, dict):
                            ...
                            errors.append(f"{rule_type}[{i}] missing required 'action' field")
    ...
    def apply_parameter_overrides(self, strategy_id: str, overrides: Dict[str, Any]) -> bool:
        """
    ...
    def get_strategy_list(self) -> List[Dict[str, Any]]:
        ...
        for strategy_config in self.load_all_strategies():
            info = {
                ...
                'last_modified': strategy_config.last_modified.isoformat() if strategy_config.last_modified else None
            }
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strategies_with_overrides': len([
                s for s in self._strategy_cache.values() 
