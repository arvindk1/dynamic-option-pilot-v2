...
class CommentaryScheduler:
        ...
        while self.running:
            try:
                ...
                if (self.last_update is None or 
                    current_time - self.last_update > timedelta(minutes=30)):
    ...
    def _update_commentary(self, update_type: str):
        ...
        try:
            ...
            commentary = self.commentary_service.get_real_time_commentary(force_refresh=True)
            
            logger.info(f"Commentary updated for {commentary.get('display_date', 'unknown date')}")
