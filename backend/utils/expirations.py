...
class ExpirationGenerator:
    ...
    def is_quarterly_expiration(friday: date) -> bool:
        ...
        return friday.month in [3, 6, 9, 12] and ExpirationGenerator.is_monthly_expiration(friday)
    ...
    def get_option_expirations(
        ...
        while len(expirations) < count and friday_count < 20:  # Safety limit
            ...
            expiration = OptionExpiration(
                date=datetime.combine(next_friday, datetime.min.time()).replace(tzinfo=timezone.utc),
                dte=dte,
                exp_type=exp_type,
...
class ExpirationCalendar:
    ...
    def adjust_for_holidays(expiration_date: date) -> date:
        ...
        if ExpirationCalendar.is_market_holiday(expiration_date):
            ...
            while (adjusted.weekday() >= 5 or 
                   ExpirationCalendar.is_market_holiday(adjusted)):
    ...
    def get_trading_days_between(start_date: date, end_date: date) -> int:
        ...
        while current <= end_date:
            if (current.weekday() < 5 and  # Not weekend
                not ExpirationCalendar.is_market_holiday(current)):
                trading_days += 1
