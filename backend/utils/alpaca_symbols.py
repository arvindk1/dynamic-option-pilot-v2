...
UNSUPPORTED_SYMBOLS: Set[str] = {
    ...
    "NDX", "^NDX", "^IXIC", # NASDAQ indexes
    "DJX", "^DJI",         # Dow Jones indexes
    "RUT", "^RUT",         # Russell indexes
    "VIX", "^VIX",         # Volatility indexes
    
    # Other common non-tradeable symbols
    "BTC-USD", "ETH-USD",  # Cryptocurrencies
    "EURUSD=X", "GBPUSD=X", # Forex pairs
}
...
def validate_symbol_for_options(symbol: str) -> tuple[bool, str, Optional[str]]:
    ...
    if normalized_symbol != original_symbol:
        if original_symbol in INDEX_TO_ETF_MAPPING:
            warning = f"Mapped index {original_symbol} to tradeable ETF {normalized_symbol}"
        else:
