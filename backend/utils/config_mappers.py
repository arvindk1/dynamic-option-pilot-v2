...
def convert_json_to_sandbox_config(json_config: Dict[str, Any], universe_loader=None) -> Dict[str, Any]:
    """
    ...
    if universe_config:
        ...
        if "universe_file" in universe_config:
            ...
            try:
                ...
                raise RuntimeError(f"Universe file '{universe_file}' could not be loaded: {e}")
            ...
            try:
                ...
                raise RuntimeError(f"Universe '{universe_name}' could not be loaded: {e}")
        
        # NO FALLBACK - explicit error if no symbols loaded
        if not symbols:
            raise RuntimeError("No symbols loaded from universe configuration - system health compromised")
    ...
    if exit_rules:
        ...
        if stop_losses:
            loss_limit = abs(stop_losses[0].get("trigger", -0.30)) * 3  # Convert to multiplier
    ...
    sandbox_config["strategy"] = {
        ...
        "risk_level": json_config.get("educational_content", {}).get("risk_level", "MEDIUM")
    }
...
def validate_strategy_config(config: Dict[str, Any], config_type: str = "sandbox") -> Dict[str, Any]:
    """
    ...
    if config_type == "sandbox":
        ...
        if not symbols:
            ...
            validation_result["warnings"].append(f"Only {len(symbols)} symbols in universe - consider adding more for diversification")
        
        # Validate trading parameters
        trading_config = config.get("trading", {})
        dte_range = trading_config.get("target_dte_range", [])
        if len(dte_range) != 2 or dte_range[0] >= dte_range[1]:
            validation_result["errors"].append("Invalid DTE range - should be [min, max] with min < max")
            validation_result["valid"] = False
