...
class OptionsDataValidator:
    ...
    async def validate_opportunity(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all([symbol, expiration_date, short_strike]):
            logger.debug(f"Missing required data for opportunity validation: {symbol} (exp: {expiration_date}, strike: {short_strike})")
            return None
        ...
        try:
            ...
            if not short_strike_data:
                logger.debug(f"Short strike {short_strike} not found in real options data for {symbol}")
                return None
            
            if long_strike and not long_strike_data:
                logger.debug(f"Long strike {long_strike} not found in real options data for {symbol}")
                return None
            ...
            real_mid = (real_bid + real_ask) / 2 if real_bid and real_ask else opportunity.get("premium", 0.0)
            ...
            if long_strike_data:
                ...
                opportunity["max_loss"] = abs(short_strike - long_strike) - spread_credit
            else:
                # Single option
            ...
            if "impliedVolatility" in short_strike_data:
                opportunity["implied_volatility"] = short_strike_data["impliedVolatility"]
    ...
    def _find_strike_data(self, options_data, target_strike: float) -> Optional[Dict]:
        ...
        for _, option in options_data.iterrows():
            ...
            if abs(option_strike - target_strike) < 0.01:  # Allow small floating point differences
                return {
    ...
    async def validate_opportunities_batch(self, opportunities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for symbol, symbol_opps in list(symbol_groups.items())[:5]:  # Max 5 symbols to avoid rate limits
            try:
        ...
        logger.info(f"Validated {len(validated_opportunities)} opportunities (real + high-confidence synthetic)")
