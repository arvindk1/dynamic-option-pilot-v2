...
def calculate_spread_greeks(
    ...
    # Delta approximation
    if option_type.upper() == "PUT":
        short_delta = -max(0.01, min(0.99, (underlying_price - short_strike) / underlying_price))
        long_delta = -max(0.01, min(0.99, (underlying_price - long_strike) / underlying_price))
    else:
        short_delta = max(0.01, min(0.99, (short_strike - underlying_price) / underlying_price))
        long_delta = max(0.01, min(0.99, (long_strike - underlying_price) / underlying_price))
    ...
    theta = -abs(net_delta) * iv * 0.1 / math.sqrt(time_to_expiry) if time_to_expiry > 0 else 0
