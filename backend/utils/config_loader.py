...
class ConfigLoader:
        ...
        for config_file in config_files:
            if config_file.exists():
                try:
                    ...
                    logger.info(
                        f"Loaded strategy config for {strategy_id} from {config_file.name}"
                    )
                    return config
    ...
    def load_universe_symbols(self, universe_name: str) -> Optional[list]:
        ...
        universe_file = (
            self.config_root.parent / "data" / "universes" / f"{universe_name}.txt"
        )
    ...
    def list_strategy_configs(self) -> list:
        ...
        for file_path in list(strategies_dir.glob("*.json")) + list(
            strategies_dir.glob("*.yaml")
        ):
            strategy_id = file_path.stem
            if (
                strategy_id not in config_files
