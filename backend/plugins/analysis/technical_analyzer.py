...
from core.orchestrator.base_plugin import AnalysisPlugin, PluginMetadata, PluginType, PluginConfig
...
def compute_bollinger_bands(series: pd.Series, period: int = 20, std_dev: float = 2) -> pd.DataFrame:
...
class TechnicalAnalyzer(AnalysisPlugin):
    ...
    async def analyze(self, data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if not price_data or (isinstance(price_data, pd.DataFrame) and price_data.empty):
        ...
        try:
            ...
            analysis_result.update(await self._analyze_market_regime(price_data, settings))
            
            # Add signal analysis
            analysis_result.update(await self._analyze_signals(price_data, analysis_result, settings))
    ...
    async def _calculate_all_indicators(self, price_data: pd.DataFrame, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "macd_signal": float(macd_df["signal"].iloc[-1]) if not macd_df.empty else 0.0,
            "macd_histogram": float(macd_df["hist"].iloc[-1]) if not macd_df.empty else 0.0,
            ...
            "bb_bandwidth": float(bb_df["bandwidth"].iloc[-1]) if not bb_df.empty else 0.0,
            "stoch_k": float(stoch_df["k_percent"].iloc[-1]) if not stoch_df.empty else 50.0,
            "stoch_d": float(stoch_df["d_percent"].iloc[-1]) if not stoch_df.empty else 50.0,
    ...
    async def _analyze_market_regime(self, price_data: pd.DataFrame, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if all(col in price_data.columns for col in ['high', 'low', 'close']):
            ...
            volatility_regime = determine_volatility_regime(atr_series, settings.get('high_vol_threshold', 0.75))
        else:
    ...
    async def _analyze_signals(self, price_data: pd.DataFrame, indicators: Dict[str, Any], settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        signals = {
            ...
            "price_above_ema_short": indicators.get("current_price", 0) > indicators.get("ema_short", 0),
            "price_above_ema_long": indicators.get("current_price", 0) > indicators.get("ema_long", 0),
    ...
    async def calculate_rsi_coupon_signals(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "price_vs_sma_pct": round((current_price / sma_50 - 1) * 100, 1) if sma_50 > 0 else 0.0,
            "rsi_exit_triggered": rsi_exit_triggered,
            "trend_break": trend_break,
            "exit_reasons": exit_reasons
        }
    
    async def get_support_resistance(self, symbol: str, price_data: Optional[pd.DataFrame] = None) -> Dict[str, float]:
