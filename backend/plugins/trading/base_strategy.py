...
from core.orchestrator.base_plugin import TradingStrategyPlugin, PluginMetadata, PluginType, PluginConfig
    ...
    def __post_init__(self):
        if self.symbols is None:
            ...
            try:
                ...
                self.symbols = ["SPY", "QQQ", "IWM"]  # Final fallback only if external loading fails
...
class BaseStrategyPlugin(TradingStrategyPlugin):
    ...
    async def scan_opportunities_v2(self, universe: List[str], **kwargs) -> List[Dict[str, Any]]:
    ...
    def _calculate_score(self, opportunity: StrategyOpportunity) -> float:
        ...
        try:
            ...
            ev_normalized = min(opportunity.expected_value / 200.0, 1.0)  # Normalize to $200 max
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            'strategy_id': self.strategy_config.strategy_id if self.strategy_config else 'unknown',
            'total_opportunities': self._performance_stats.get('total_opportunities', 0),
    ...
    def update_performance_stats(self, trade_result: Dict[str, Any]):
        ...
        try:
            # Update trade count
            self._performance_stats['total_opportunities'] = self._performance_stats.get('total_opportunities', 0) + 1
            
            # Update success metrics
            if trade_result.get('success', False):
                self._performance_stats['successful_trades'] = self._performance_stats.get('successful_trades', 0) + 1
            
            # Calculate win rate
            total = self._performance_stats['total_opportunities']
            successful = self._performance_stats['successful_trades']
            self._performance_stats['win_rate'] = successful / total if total > 0 else 0.0
            
            # Update profit metrics
            profit = trade_result.get('profit', 0.0)
            current_avg = self._performance_stats.get('average_profit', 0.0)
            self._performance_stats['average_profit'] = (current_avg * (total - 1) + profit) / total
...
class V1StrategyMigrationMixin:
    ...
    def format_v1_opportunity(self, symbol: str, strategy_type: str, **kwargs) -> StrategyOpportunity:
        """Format opportunity in V1 style for easier migration."""
        opportunity_id = f"{strategy_type}_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def validate_liquidity_requirements(self, opportunity: StrategyOpportunity, min_score: float = 7.0) -> bool:
        """Validate liquidity requirements - common V1 pattern."""
        return opportunity.liquidity_score >= min_score
    
    def validate_probability_requirements(self, opportunity: StrategyOpportunity, min_prob: float = 0.50) -> bool:
