...
class ThetaCropWeeklyPlugin(BaseStrategyPlugin, V1StrategyMigrationMixin):
    ...
    def _load_strategy_config(self) -> StrategyConfig:
        ...
        try:
            ...
            return StrategyConfig(
                ...
                min_liquidity_score=config_data["market_conditions"][
                    "min_liquidity_score"
                ],
        ...
        try:
            ...
            try:
                ...
                return [
                    ...
                    "QQQ",
                    "IWM",
                ]  # Final hardcoded fallback only if everything fails
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"ThetaCrop Weekly initialized with {len(self._universe_symbols)} symbols"
            )
            return True
    ...
    async def validate_opportunity(self, opportunity: StrategyOpportunity) -> bool:
        ...
        try:
            ...
            # Check DTE range
            if not (
                self.strategy_config.min_dte
                <= opportunity.days_to_expiration
                <= self.strategy_config.max_dte
            ):
                return False
            ...
            if not self.validate_liquidity_requirements(
                opportunity, self.strategy_config.min_liquidity_score
            ):
                return False
        ...
        try:
            ...
            if opportunity.max_loss > 0:
                metrics["risk_reward_ratio"] = (
                    opportunity.max_profit / opportunity.max_loss
                )
                metrics["credit_to_width_ratio"] = (
                    opportunity.premium / opportunity.max_loss
                )
    ...
    async def _convert_setup_to_opportunity(
        self, setup: IronCondorSetup
        ...
        try:
            ...
            opportunity = StrategyOpportunity(
                ...
                short_strike=(setup.put_short_strike + setup.call_short_strike)
                / 2,  # Average
                ...
                probability_profit=self._calculate_probability_from_delta(
                    0.20
                ),  # ±20 delta
                expected_value=setup.max_profit
                * self._calculate_probability_from_delta(0.20),
                # Greeks
    ...
    async def _estimate_delta_strikes(
        self, symbol: str, price: float, expiration: datetime, delta: float
    ...
    async def _create_condor_setup(
        self,
        symbol: str,
        expiration: datetime,
        dte: int,
