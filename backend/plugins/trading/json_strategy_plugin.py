...
from core.interfaces.data_provider_interface import DataProviderType, IDataProvider
...
class JSONStrategyPlugin(BaseStrategyPlugin):
    ...
    async def scan_opportunities(
        ...
        try:
            ...
            if not data_provider:
                logger.error(
                    f"No data provider available for {self.json_config.strategy_id}"
                )
                return []

            # Apply universe filtering from JSON config
            filtered_universe = await self._filter_universe(
                universe, data_provider, effective_config
            )

            # Scan each symbol using JSON-defined rules
            opportunities = []
            for symbol in filtered_universe:
                symbol_opportunities = await self._scan_symbol(
                    symbol, data_provider, effective_config
                )
                opportunities.extend(symbol_opportunities)

            # Apply JSON-defined scoring and ranking
            scored_opportunities = self._apply_json_scoring(
                opportunities, effective_config
            )

            # Limit results per JSON config
            max_opps = effective_config.get("position_parameters", {}).get(
                "max_opportunities", 10
            )
            final_opportunities = sorted(
                scored_opportunities, key=lambda x: x.score, reverse=True
            )[:max_opps]
            ...
            logger.info(
                f"JSON strategy {self.json_config.strategy_id} found {len(final_opportunities)} opportunities"
            )
            return final_opportunities

        except Exception as e:
            logger.error(
                f"Error scanning with JSON strategy {self.json_config.strategy_id}: {e}"
            )
            return []

    async def _filter_universe(
        self, universe: List[str], data_provider: IDataProvider, config: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _scan_symbol(
        self, symbol: str, data_provider: IDataProvider, config: Dict[str, Any]
    ) -> List[StrategyOpportunity]:
        ...
        try:
            ...
            opportunities = await self._generate_opportunities(
                symbol, quote, data_provider, config
            )
    ...
    async def _passes_entry_signals(
        self, symbol: str, quote, config: Dict[str, Any]
    ) -> bool:
        ...
        if strategy_type == "RSI_COUPON":
            ...
            simulated_iv = (
                0.2 + (hash(symbol) % 20) / 100
            )  # Simulated IV between 0.2-0.4
    ...
    async def _generate_opportunities(
        self, symbol: str, quote, data_provider: IDataProvider, config: Dict[str, Any]
    ) -> List[StrategyOpportunity]:
    ...
    async def _create_opportunity(
        self, symbol: str, quote, config: Dict[str, Any], variant: int = 0
    ) -> Optional[StrategyOpportunity]:
        ...
        try:
            ...
            if strategy_type == "IRON_CONDOR":
                ...
                protection_level = position_params.get(
                ...
                if (hash(f"{symbol}_{variant}") % 2) == 0:
                    short_strike_call = quote.price * (
                        1 + delta_target + variant * 0.02
                    )
            ...
            # Calculate metrics
            probability_profit = (
                0.65 + (hash(f"{symbol}_{variant}") % 30) / 100
            )  # 0.65-0.95
            expected_value = premium * 100 * probability_profit - max_loss * (
                1 - probability_profit
            )
            ...
            opportunity = StrategyOpportunity(
                ...
                short_strike=(
                    short_strike_put if short_strike_put else short_strike_call
                ),
                long_strike=(
                    long_strike_put
                    if strategy_type == "PROTECTIVE_PUT"
                    and "long_strike_put" in locals()
                    else (
                        (short_strike_put - 10)
                        if short_strike_put
                        else ((short_strike_call + 10) if short_strike_call else None)
                    )
                ),
                # Financial metrics
                premium=premium,
                ...
                days_to_expiration=self._calculate_proper_dte(
                    position_params.get("target_dtes", [30])[0]
                ),
                expiration=self._calculate_expiration_date(
                    position_params.get("target_dtes", [30])[0]
                ),
                volume=quote.volume,
    ...
    def _apply_json_scoring(
        self, opportunities: List[StrategyOpportunity], config: Dict[str, Any]
    ) -> List[StrategyOpportunity]:
    ...
    def calculate_position_size(
        ...
        try:
            ...
            position_size_per_3k = effective_config.get("position_parameters", {}).get(
            ...
            position_size = min(
                max_contracts, scaled_contracts, 10
            )  # Cap at 10 contracts
    ...
    async def initialize(self) -> bool:
        ...
        try:
            logger.info(
                f"Initializing JSON strategy plugin: {self.json_config.strategy_id}"
            )
            ...
            if not effective_config:
                logger.error(
                    f"Invalid configuration for strategy {self.json_config.strategy_id}"
                )
                return False
            ...
            if not self.engine_registry:
                logger.error(
                    f"Engine registry not available for strategy {self.json_config.strategy_id}"
                )
                return False
            ...
            if not data_provider:
                logger.warning(
                    f"No data provider available for strategy {self.json_config.strategy_id} - will use runtime provider"
                )
            ...
            logger.info(
                f"Successfully initialized JSON strategy: {self.json_config.strategy_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to initialize JSON strategy {self.json_config.strategy_id}: {e}"
            )
            return False
    ...
    async def cleanup(self) -> bool:
        ...
        try:
            logger.info(
                f"Cleaning up JSON strategy plugin: {self.json_config.strategy_id}"
            )
            ...
            logger.info(
                f"Successfully cleaned up JSON strategy: {self.json_config.strategy_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to cleanup JSON strategy {self.json_config.strategy_id}: {e}"
            )
            return False

    async def calculate_risk_metrics(
        self, opportunity: StrategyOpportunity, account_size: float = 10000
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Risk metrics
            risk_reward_ratio = (
                total_premium / total_max_loss if total_max_loss > 0 else 0
            )
            ...
            time_risk = (
                "LOW"
                if opportunity.days_to_expiration > 30
                else "MEDIUM" if opportunity.days_to_expiration > 14 else "HIGH"
            )

            # Liquidity risk
            liquidity_risk = (
                "LOW"
                if opportunity.liquidity_score > 7
                else "MEDIUM" if opportunity.liquidity_score > 4 else "HIGH"
            )
            ...
            return {
                ...
                "max_recommended_allocation": risk_mgmt.get(
