...
class YFinanceProvider(DataProviderPlugin, IDataProvider):
    ...
    async def _get_cached_or_fetch(
        self, cache_key: str, data_type: str, fetch_func
    ) -> Any:
        ...
        try:
            ...
            self.cache[cache_key] = CachedData(
                data=data, timestamp=now, expires_at=expires_at
            )
    ...
    async def get_market_data(self, symbol: str, **kwargs) -> Dict[str, Any]:
        ...
        async def fetch_market_data():
            ...
            try:
                ...
                if info.get("preMarketPrice") and info.get("preMarketPrice") > 0:
                    ...
                    and info.get("regularMarketPrice") > 0
                ):
        ...
        return await self._get_cached_or_fetch(
            cache_key, "market_data", fetch_market_data
        )
    ...
    async def get_options_chain(self, symbol: str, **kwargs) -> Dict[str, Any]:
        ...
        cache_key = self._get_cache_key(
            "option_chain", symbol, expiration=expiration.strftime("%Y-%m-%d")
        )
        yf_symbol = self._map_symbol(symbol)
        ...
        return await self._get_cached_or_fetch(
            cache_key, "option_chains", fetch_option_chain
        )
    ...
    async def get_historical_data(self, symbol: str, **kwargs) -> List[Dict]:
                    ...
                    result.append(
                        {
                            ...
                            "open": (
                                float(row.get("Open", 0).iloc[0])
                                if hasattr(row.get("Open", 0), "iloc")
                                else float(row.get("Open", 0))
                            ),
                            "high": (
                                float(row.get("High", 0).iloc[0])
                                if hasattr(row.get("High", 0), "iloc")
                                else float(row.get("High", 0))
                            ),
                            "low": (
                                float(row.get("Low", 0).iloc[0])
                                if hasattr(row.get("Low", 0), "iloc")
                                else float(row.get("Low", 0))
                            ),
                            "close": (
                                float(row.get("Close", 0).iloc[0])
                                if hasattr(row.get("Close", 0), "iloc")
                                else float(row.get("Close", 0))
                            ),
                            "volume": (
                                int(row.get("Volume", 0).iloc[0])
                                if hasattr(row.get("Volume", 0), "iloc")
        ...
        return await self._get_cached_or_fetch(
            cache_key, "historical", fetch_historical
        )
