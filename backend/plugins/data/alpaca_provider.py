...
class AlpacaProvider(DataProviderPlugin, IDataProvider):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self._logger.info(
                f"✅ Connected to Alpaca {'paper' if self.paper else 'live'} account: {account.id}"
            )
            return True
        ...
        try:
            ...
            if symbol.upper() in ["SPY"]:
                try:
                    ...
                    vix = (
                        (vix_quote.bid_price + vix_quote.ask_price) / 2
                        if vix_quote
                        else 16.5
                    )
                except:
        ...
        try:
            ...
            for position in positions:
                ...
                position_dict = {
                    ...
                    "market_value": (
                        float(position.market_value) if position.market_value else 0.0
                    ),
                    "pnl": (
                        float(position.unrealized_pl) if position.unrealized_pl else 0.0
                    ),
                    "pnl_percentage": (
                ...
                if len(position.symbol) > 6 and any(
                    char in position.symbol for char in ["C", "P"]
                ):
            ...
            self._logger.info(
                f"✅ Retrieved {len(position_data)} positions from Alpaca"
            )
    ...
    async def get_account_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_pnl": (
                    float(account.unrealized_pl) if account.unrealized_pl else 0.0
                ),
