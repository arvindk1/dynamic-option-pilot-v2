    ...
    try:
        ...
        for json_file in strategies_dir.glob("*.json"):
            try:
                ...
                strategy_exists = any(s.name == strategy_name for s in existing_strategies)
                ...
                if not strategy_exists:
                    ...
                    logger.info(f"Created sandbox strategy: {strategy_name} from {json_file.name}")
                else:
                    # Update existing strategy with latest JSON config
                    existing_strategy = next(s for s in existing_strategies if s.name == strategy_name)
                    updated_config = await sandbox_service.update_strategy_config(
                        existing_strategy.id,
                        {"config_data": sandbox_config}
                    )
                    logger.info(f"Updated sandbox strategy: {strategy_name} from {json_file.name}")
...
async def sync_sandbox_strategy_with_json(strategy_id: str, json_file_path: str):
    ...
    try:
        ...
        # Update the strategy
        updated = await sandbox_service.update_strategy_config(strategy_id, {
            "config_data": sandbox_config
        })
        
        if updated:
            logger.info(f"Successfully synced sandbox strategy {strategy_id} with JSON file")
            return True
        else:
