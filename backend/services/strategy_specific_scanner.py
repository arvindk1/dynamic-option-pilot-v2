...
class StrategySpecificScanner:
    ...
    def _load_strategy_configs(self) -> Dict[str, Dict[str, Any]]:
        ...
        configs = {}
        strategy_dir = Path(
            "/home/arvindk/devl/dynamic-option-pilot-v2/backend/config/strategies/production"
        )
    ...
    async def scan_strategy(
        ...
        try:
            ...
            if not base_config:
                raise RuntimeError(
                    f"Strategy configuration not found for {strategy_type}"
                )
            ...
            await log_critical_error(
                ...
                details={"strategy_type": strategy_type, "error": str(e)},
    ...
    def _merge_configs(
        self, base_config: Dict[str, Any], sandbox_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _get_strategy_symbols(
        self, config: Dict[str, Any], test_params: Optional[Dict[str, Any]] = None
    ) -> List[str]:
        ...
        if test_params or not universe_config:
            logger.warning(
                f"No universe configuration found, using default symbols for testing"
            )
    ...
    async def _generate_strategy_opportunities(
        self,
        strategy_type: str,
        config: Dict[str, Any],
        symbols: List[str],
        ...
        opportunities = []
        max_opportunities = (
            test_params.get("max_opportunities", 5) if test_params else 5
        )
    ...
    async def _generate_theta_harvesting_opportunity(
        self,
        symbol: str,
        ...
        return {
            ...
            "expected_value": round(
                credit * probability_profit
                - (max_loss - credit) * (1 - probability_profit),
                2,
            ),
            ...
            "score": round(
                min(
                    10.0, max(1.0, probability_profit * base_prob_weight + theta_bonus)
                ),
                1,
            ),
    ...
    async def _generate_rsi_coupon_opportunity(
        self,
        symbol: str,
        position_params: Dict,
        entry_signals: Dict,
        ...
        underlying_price = base_prices.get(symbol, 120) + random.uniform(
            -10, 5
        )  # Slight downward bias for oversold
            ...
            "score": round(
                min(
                    ...
                    max(
                        2.0,
                        probability_profit
                        * scoring.get("base_probability_weight", 5.0)
                        * rsi_bonus,
                    ),
                ),
                1,
    ...
    async def _generate_iron_condor_opportunity(
        self,
        symbol: str,
        position_params: Dict,
        entry_signals: Dict,
    ...
    async def _generate_protective_put_opportunity(
        self,
        symbol: str,
        position_params: Dict,
        entry_signals: Dict,
    ...
    async def _generate_volatility_opportunity(
        self,
        symbol: str,
        position_params: Dict,
        entry_signals: Dict,
        ...
        return {
            ...
            "score": round(
                min(
                    10.0,
                    max(3.0, (iv_rank / 50) * scoring.get("volatility_weight", 3.0)),
                ),
                1,
            ),
            "created_at": datetime.utcnow().isoformat(),
    ...
    async def _generate_generic_opportunity(
        self,
        symbol: str,
        position_params: Dict,
        entry_signals: Dict,
        ...
        underlying_price = base_prices.get(
            symbol, base_prices["default"]
