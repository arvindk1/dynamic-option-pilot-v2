...
class UniverseLoader:
        ...
        try:
            ...
            for file_path in self.universes_dir.glob("*.txt"):
                ...
                if symbols:
                    ...
                    logger.info(
                        f"Loaded universe '{universe_name}': {len(symbols)} symbols"
                    )
    ...
    def get_strategy_universe_priority(self, strategy: str) -> List[str]:
        ...
        if strategy == "quick_scalp":
            ...
            other_symbols = [
                s for s in symbols if s not in mag7 and s not in major_etfs
            ]
            ...
            try:
                ...
                raise RuntimeError(
                    f"Universe file 'thetacrop_symbols' could not be loaded: {e}. System health compromised."
                )
