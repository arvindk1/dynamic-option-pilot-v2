...
class OpportunityCache:
        ...
        try:
            ...
            if self.plugin_registry:
                ...
                if live_result is not None:
                    ...
                    logger.info(
                        f"Live scan completed for {cache_key}: {len(live_result)} opportunities"
                    )
                    # Update both caches
            ...
            logger.warning(
                f"No opportunities found for {cache_key} - all cache layers empty"
            )
            return []
    ...
    async def add_opportunities(
        ...
        try:
            ...
            logger.info(
                f"Added {len(opportunities)} opportunities to cache for strategy {strategy}"
            )
        ...
        try:
            ...
            try:
                expired_opportunities = (
                    ...
                    .filter(OpportunitySnapshot.expires_at < current_time)
                    .all()
                )
                ...
                if expired_keys or expired_opportunities:
                    logger.info(
                        f"Cleaned up {len(expired_keys)} memory entries and "
    ...
    def _generate_cache_key(
        self, strategy: str = None, symbols: List[str] = None
    ) -> str:
    ...
    def _get_from_memory(
        self, cache_key: str, strategy: str = None
        ...
        if cache_key in self.cache_timestamps:
            age_seconds = (
                datetime.utcnow() - self.cache_timestamps[cache_key]
            ).total_seconds()
        ...
        try:
            ...
            try:
                ...
                opportunities = query.order_by(
                    desc(OpportunitySnapshot.created_at)
                ).all()
    ...
    async def _perform_live_scan(
        ...
        logger.info(
            f"DEBUG: _perform_live_scan called with strategy={strategy}, symbols={symbols}"
        )
        try:
            ...
            if not strategy_registry:
                ...
                raise RuntimeError(
                    f"Strategy registry not available - system health compromised"
                )
            ...
            if symbols:
                ...
                scan_symbols = universe_loader.get_strategy_universe_priority(
                    strategy or "high_probability"
                )
                scan_symbols = scan_symbols[:8]  # Limit for performance

            logger.info(
                f"Performing live scan for strategy={strategy}, symbols={scan_symbols}"
            )
            ...
            if strategy and strategy != "high_probability":
                ...
                if strategy_instance:
                    ...
                    opportunities = await strategy_instance.scan_opportunities(
                        scan_symbols
                    )
                    ...
                    if result:
                        logger.info(
                            f"Strategy plugin scan found {len(result)} opportunities"
                        )
                        return result
                    else:
                    ...
                    logger.warning(
                        f"Strategy {strategy} not found in registry, using fallback"
                    )
                    # No fallback - log error and fail explicitly
                await log_critical_error(
                    error_type="strategy_registry_unavailable",
            ...
            all_opportunities_by_strategy = await strategy_registry.scan_all_strategies(
                scan_symbols
            )

            # Flatten results and convert to API format
            result = []
            for strategy_id, opps in all_opportunities_by_strategy.items():
                for opp in opps:
                    opp_dict = {
                        "id": opp.id,
                        "symbol": opp.symbol,
            ...
            if result:
                logger.info(
                    f"Strategy registry scan found {len(result)} opportunities across {len(all_opportunities_by_strategy)} strategies"
                )
                return result
            else:
    ...
    async def _fallback_scan(
        ...
        raise RuntimeError(
            f"Strategy {strategy} is not available. System health compromised - check strategy registry."
        )
    ...
    async def _get_real_price(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.error(
                f"Market data unavailable for {symbol} - no fallback price provided: {e}"
            )
            raise RuntimeError(
                f"Market data provider failed for {symbol}. System health compromised."
            )
