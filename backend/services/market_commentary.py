...
class MarketCommentaryService:
    ...
    def generate_earnings_preview(self) -> List[str]:
        ...
        potential_earnings = [
            {"symbol": "AAPL", "when": "After Market Close", "consensus": "Beat Expected"},
            {"symbol": "MSFT", "when": "Before Market Open", "consensus": "Mixed Expectations"},
            {"symbol": "GOOGL", "when": "After Market Close", "consensus": "Revenue Growth Focus"},
            {"symbol": "NVDA", "when": "After Market Close", "consensus": "AI Demand Strong"},
    ...
    def generate_session_specific_content(self, session: MarketSession) -> Dict[str, Any]:
    ...
    def generate_dynamic_themes(self) -> List[str]:
        ...
        if earnings_themes:
            base_themes.extend([f"Earnings focus: {theme}" for theme in earnings_themes])
        
        # Add session-specific themes
        if session == MarketSession.PRE_MARKET:
            base_themes.append("Pre-market indicators suggesting potential gap moves")
        elif session == MarketSession.REGULAR_HOURS:
            base_themes.append("Intraday momentum shifts creating options opportunities")
        elif session == MarketSession.AFTER_HOURS:
            base_themes.append("After-hours earnings reactions setting up extended moves")
