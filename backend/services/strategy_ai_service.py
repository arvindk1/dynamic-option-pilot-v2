...
class StrategyAIService:
    ...
    def _check_rate_limits(self) -> bool:
        ...
        if (
            ...
            logger.warning(
                f"Rate limit exceeded: {len(self.request_timestamps)} requests in the last minute"
            )
            return False
        ...
        if self.daily_request_count >= self.rate_limit_config["requests_per_day"]:
            logger.warning(
                f"Daily rate limit exceeded: {self.daily_request_count} requests today"
            )
            return False
    ...
    async def _make_openai_request_with_retry(
        self, messages: List[Dict], attempt: int = 1
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "rate_limit" in error_str or "429" in error_str:
                if attempt <= self.rate_limit_config["max_retries"]:
                    ...
                    logger.warning(
                        f"Rate limit hit (attempt {attempt}/{self.rate_limit_config['max_retries']}), "
                        f"retrying in {delay:.1f} seconds..."
                    )
                    await asyncio.sleep(delay)
                    return await self._make_openai_request_with_retry(
                        messages, attempt + 1
                    )
                else:
                    ...
                    if self.current_model != "gpt-3.5-turbo":
                        logger.warning(
                            "Max retries exceeded, switching to gpt-3.5-turbo as fallback"
                        )
                        old_model = self.current_model
                        self.current_model = "gpt-3.5-turbo"
                        try:
                            result = await self._make_openai_request_with_retry(
                                messages, 1
                            )
                            ...
                            raise Exception(
                                f"Rate limit exceeded. Please try again in a few minutes. "
                                f"Error: {str(e)[:100]}..."
                            )
                    else:
                        raise Exception(
                            f"Rate limit exceeded on all models. Please try again in a few minutes. "
                            f"Error: {str(e)[:100]}..."
                        )
            ...
            # Handle other API errors
                ...
                if attempt <= self.rate_limit_config["max_retries"]:
                    ...
                    logger.warning(
                        f"API error (attempt {attempt}), retrying in {delay:.1f} seconds: {str(e)[:100]}"
                    )
                    await asyncio.sleep(delay)
                    return await self._make_openai_request_with_retry(
                        messages, attempt + 1
                    ...
                    raise Exception(
                        f"API error after {self.rate_limit_config['max_retries']} attempts: {str(e)[:100]}..."
                    )
...
You are an expert options strategy analyst focused solely on optimizing this {strategy_name} strategy."""
    ...
    async def chat_with_strategy(
        ...
        """
        logger.info(
            f"AI Chat Request - Strategy: {strategy_name}, Message: {user_message[:50]}..."
        )
        logger.info(
            f"AI Service State - Enabled: {self.enabled}, Client: {self.client is not None}"
        )
        ...
        try:
            ...
            system_prompt = self._create_strategy_prompt(
                strategy_name, strategy_config, performance_data
            )

            # Build conversation history
            conversation_id = (
                conversation_id or f"{strategy_name}_{datetime.now().isoformat()}"
            )
            ...
            logger.info(
                f"Strategy AI chat - Model: {self.current_model}, "
