...
class SandboxDataCache:
    ...
    async def get_cached_market_data(
        self, symbols: List[str], db: Session
    ) -> Dict[str, Any]:
        ...
        for symbol in symbols:
            ...
            cached_entry = (
                db.query(SandboxHistoricalCache)
                .filter(
    ...
    async def get_cached_options_chain(
        self, symbol: str, expiration: str, db: Session
    ) -> Dict[str, Any]:
    ...
    def _generate_sample_options_chain(
        ...
        for strike in strikes:
            ...
            call_premium = (
                ...
                else max(underlying_price - strike, base_premium)
            )
            put_premium = (
                base_premium
                if strike < underlying_price
                else max(strike - underlying_price, base_premium)
            )
...
class SandboxTestEngine:
    ...
    async def run_strategy_test(
        self, config: SandboxStrategyConfig, test_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                config_file_path = (
                    ...
                    / "strategies"
                    / "development"
                    / f"{config.strategy_id}.json"
                )
                if config_file_path.exists():
                    with open(config_file_path, "r") as f:
                        base_config = json.load(f)
                    strategy_type = base_config.get(
                        "strategy_type", config.strategy_id.upper()
                    )
                    logger.info(
                        f"Loaded strategy_type '{strategy_type}' for {config.strategy_id} from config file"
                    )
                else:
                    # Fallback: try to get from sandbox config or use strategy_id
                    strategy_info = strategy_config_data.get("strategy", {})
                    strategy_type = strategy_info.get(
                        "strategy_type", config.strategy_id.upper()
                    )
                    logger.warning(
                        f"Config file not found for {config.strategy_id}, using fallback: {strategy_type}"
                    )
            except Exception as e:
                # Final fallback
                logger.warning(
                    f"Could not determine strategy_type for {config.strategy_id}: {e}"
                )
                strategy_type = config.strategy_id.upper()
            ...
            if not scan_result.get("success", False):
                return self._create_error_result(
                    scan_result.get("error", "Strategy scanning failed")
                )
            ...
            logger.info(
                f"Strategy-specific scan completed for {strategy_type}: {len(opportunities)} opportunities found"
            )
            ...
            try:
                ...
                logger.info(
                    f"Sandbox test completed: {len(opportunities)} opportunities found in {execution_time}ms"
                )
    ...
    async def _execute_strategy_with_cached_data(
        self,
        strategy_instance,
        symbols: List[str],
    ...
    def _opportunity_to_dict(self, opp: StrategyOpportunity) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_bias": getattr(
                opp, "bias", "NEUTRAL"
            ),  # Use bias instead of market_bias
            "universe": getattr(opp, "universe", "default"),  # Handle missing universe
            "created_at": (
                opp.generated_at.isoformat()
                if hasattr(opp, "generated_at") and opp.generated_at
    ...
    def _calculate_performance_metrics(
        ...
        for opp in opportunities:
            ...
            if ev == 0.0:
                premium = opp.get(
                    "premium", opp.get("premium_paid", opp.get("insurance_cost", 0.0))
                )
                max_loss = opp.get("max_loss", premium)
                if isinstance(max_loss, str):  # Handle "Unlimited" cases
                    max_loss = premium
                ev = (
                    premium * prob - max_loss * (1 - prob)
                    if max_loss > 0
                    else premium * prob
                )
        ...
        return {
            ...
            "symbols_with_opportunities": len(
                set(opp.get("symbol", "Unknown") for opp in opportunities)
            ),
        }
...
class SandboxService:
    ...
    async def get_user_strategies(
        ...
        try:
            configs = (
                db.query(SandboxStrategyConfig)
                .filter(SandboxStrategyConfig.user_id == user_id)
                .order_by(desc(SandboxStrategyConfig.updated_at))
                .all()
            )
            ...
            for config in configs:
                ...
                if key not in seen_strategies:
                    ...
                    logger.warning(
                        f"Skipping duplicate sandbox strategy: {config.id} ({config.name})"
                    )
    ...
    async def get_strategy_config(
        self, config_id: str
    ) -> Optional[SandboxStrategyConfig]:
        ...
        try:
            config = (
                db.query(SandboxStrategyConfig)
                .filter(SandboxStrategyConfig.id == config_id)
                .first()
            )
    ...
    async def run_strategy_test(
        self, config_id: str, test_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    async def run_batch_parameter_test(
        self, config_id: str, parameter_sets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for result in batch_results:
            if result.get("success") and result.get("performance_metrics"):
                ...
                score = metrics.get("avg_expected_value", 0) * metrics.get(
        ...
        return {
            ...
            "summary": {
                "total_opportunities": sum(
                    r.get("opportunities_count", 0) for r in batch_results
                ),
                "avg_opportunities_per_test": sum(
                    r.get("opportunities_count", 0) for r in batch_results
                )
                / len(batch_results),
    ...
    async def get_test_results(
        ...
        try:
            results = (
                db.query(SandboxTestRun)
                .filter(SandboxTestRun.config_id == config_id)
                .order_by(desc(SandboxTestRun.created_at))
                .limit(limit)
                .all()
            )
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            old_tests = (
                db.query(SandboxTestRun)
                .filter(SandboxTestRun.created_at < old_date)
                .all()
            )
            ...
            logger.info(
                f"Cleaned up {expired_cache_count} cache entries and {old_test_count} old test runs"
            )
