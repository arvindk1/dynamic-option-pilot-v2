...
async def log_critical_error(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"CRITICAL ERROR LOGGED: {error_type} - {message[:100]}... [ID: {error_log.id}]"
            )
...
async def get_critical_errors(
    ...
    try:
        ...
        try:
            ...
            errors = (
                query.order_by(CriticalErrorLog.created_at.desc()).limit(limit).all()
            )
...
async def mark_error_resolved(error_id: str, resolution_notes: str = None) -> bool:
    ...
    try:
        ...
        try:
            error_log = (
                db.query(CriticalErrorLog)
                .filter(CriticalErrorLog.id == error_id)
                .first()
            )
    ...
    try:
        ...
        try:
            ...
            recent_errors = await get_critical_errors(
                limit=10, unresolved_only=True, since_hours=24
            )

            # Get latest health status for each service
            latest_health = (
                db.query(SystemHealthStatus)
                .order_by(
            ...
            error_counts = (
                db.query(CriticalErrorLog)
                .filter(CriticalErrorLog.created_at >= since_24h)
                .all()
            )
