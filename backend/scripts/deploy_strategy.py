...
class StrategyDeployer:
        ...
        for env in envs_to_check:
            ...
            for strategy_file in strategies:
                try:
                    ...
                    print(f"      Universe: {universe_file} (max {max_symbols} symbols)")
    ...
    def validate_strategy(self, strategy_path):
        ...
        try:
            ...
            required_fields = [
                "strategy_name", "strategy_type", "universe",
                "position_parameters", "entry_signals"
            ]
...
Examples:
  ...
  python deploy_strategy.py set-env production
    ...
    list_parser.add_argument("--environment", default="all", 
                           choices=["all", "development", "production", "sandbox"],
                           help="Environment to list")
    
    # Promote command
    promote_parser = subparsers.add_parser("promote", help="Promote strategy between environments")
    promote_parser.add_argument("strategy_name", help="Name of strategy to promote")
    promote_parser.add_argument("--from", dest="from_env", required=True,
                               choices=["development", "production", "sandbox"],
                               help="Source environment")
    promote_parser.add_argument("--to", dest="to_env", required=True,
                               choices=["development", "production", "sandbox"],
