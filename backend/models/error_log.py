...
class CriticalErrorLog(Base):
    ...
    )  # universe_loading_failed, ai_service_error, etc.
...
class SystemHealthStatus(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "status": self.status,
            "response_time_ms": (
                int(self.response_time_ms) if self.response_time_ms else None
            ),
            "error_count_24h": (
                int(self.error_count_24h) if self.error_count_24h else None
            ),
            "details": self.details,
        }
