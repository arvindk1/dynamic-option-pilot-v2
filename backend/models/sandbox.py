...
class SandboxStrategyConfig(Base):
    ...
    def from_strategy_config(
        ...
        return cls(
            strategy_id=strategy_id, name=name, config_data=config_data, user_id=user_id
        )
...
class SandboxTestRun(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    config_id = Column(
        String, ForeignKey("sandbox_strategy_configs.id"), nullable=False
    )
...
class SandboxHistoricalCache(Base):
    ...
    )  # 'market_data', 'options_chain', 'historical_prices'
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_expired": (
                datetime.utcnow() > self.expires_at if self.expires_at else True
            ),
        }

    @classmethod
    def create_cache_entry(
        cls, symbol: str, data_type: str, data: Dict[str, Any], ttl_hours: int = 24
...
def get_user_sandbox_stats(db, user_id: str = "default_user") -> Dict[str, Any]:
    ...
    test_runs = (
        ...
        .join(SandboxStrategyConfig)
        .filter(SandboxStrategyConfig.user_id == user_id)
        .all()
    )

    ai_conversations = (
        db.query(SandboxAIConversation)
        .join(SandboxStrategyConfig)
        .filter(SandboxStrategyConfig.user_id == user_id)
        .all()
    )
    ...
    return {
        ...
        "last_activity": (
            max([c.updated_at for c in configs] + [tr.created_at for tr in test_runs])
            if configs or test_runs
            else None
        ),
    }
...
def cleanup_expired_cache(db) -> int:
    ...
    expired_entries = (
        ...
        .filter(SandboxHistoricalCache.expires_at < datetime.utcnow())
        .all()
    )
