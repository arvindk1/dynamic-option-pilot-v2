...
from services.strategy_ai_service import StrategyAIService, get_strategy_ai_service
...
class StrategyConfigRequest(BaseModel):
    strategy_id: str = Field(
        ..., description="Strategy identifier (e.g., 'thetacrop_weekly')"
    )
...
class BatchTestRequest(BaseModel):
    parameter_sets: List[Dict[str, Any]] = Field(
        ..., description="List of parameter sets to test"
    )
...
class AIMessageRequest(BaseModel):
    ...
    include_context: bool = Field(
        True, description="Include strategy context in AI request"
    )
...
async def get_strategy_config(
    config_id: str, sandbox_service: SandboxService = Depends(get_sandbox_service_dep)
):
    ...
    try:
        ...
        if not config:
            raise HTTPException(
                status_code=404, detail=f"Strategy configuration {config_id} not found"
            )
...
async def delete_strategy_config(config_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=501, detail="Strategy deletion not implemented yet"
        )
...
async def run_batch_parameter_test(config_id: str, batch_request: BatchTestRequest):
    ...
    try:
        ...
        if not batch_request.parameter_sets:
            raise HTTPException(
                status_code=400, detail="At least one parameter set required"
            )

        if len(batch_request.parameter_sets) > 10:
            raise HTTPException(
                status_code=400, detail="Maximum 10 parameter sets allowed per batch"
            )

        results = await sandbox_service.run_batch_parameter_test(
            config_id, batch_request.parameter_sets
        )
...
async def get_test_results(
    config_id: str, limit: int = Query(10, description="Number of results to return")
):
...
async def get_test_history(
    config_id: str, limit: int = Query(20, description="Number of history items")
):
            ...
            history.append(
                {
                    ...
                    "performance_summary": result.test_results.get(
                        "performance_metrics", {}
                    ),
                }
            )
...
async def preview_parameter_changes(config_id: str, parameter_changes: Dict[str, Any]):
    ...
    try:
        ...
        current_universe = config.config_data.get("universe", {}).get(
            "primary_symbols", []
        )
        new_universe = preview_config_data.get("universe", {}).get(
            "primary_symbols", []
        )
        ...
        dte_range = preview_config_data.get("trading", {}).get(
            "target_dte_range", [7, 28]
        )
        dte_range_size = dte_range[1] - dte_range[0] if len(dte_range) >= 2 else 21
        dte_multiplier = min(
            1.0, dte_range_size / 21.0
        )  # Longer ranges = more opportunities

        max_positions = preview_config_data.get("trading", {}).get("max_positions", 3)

        estimated_opportunities = int(
            universe_size * base_opportunities_per_symbol * dte_multiplier
        )
        estimated_opportunities = min(
            estimated_opportunities, max_positions * 2
        )  # Cap based on position limits
...
async def compare_parameter_sets(
    ...
    try:
        ...
        batch_results = await sandbox_service.run_batch_parameter_test(
            config_id, batch_request
        )

        if (
            not batch_results.get("success")
            or len(batch_results.get("batch_results", [])) < 2
        ):
            raise HTTPException(
                status_code=500, detail="Failed to run comparison tests"
            )
        ...
        comparison = {
            ...
            "performance_comparison": {
                "opportunities_count": {
                    ...
                    "difference": result_b.get("opportunities_count", 0)
                    - result_a.get("opportunities_count", 0),
                    "winner": (
                        "b"
                        if result_b.get("opportunities_count", 0)
                        > result_a.get("opportunities_count", 0)
                        else "a"
                    ),
                },
                "avg_probability_profit": {
                    ...
                    "difference": metrics_b.get("avg_probability_profit", 0)
                    - metrics_a.get("avg_probability_profit", 0),
                    "winner": (
                        "b"
                        if metrics_b.get("avg_probability_profit", 0)
                        > metrics_a.get("avg_probability_profit", 0)
                        else "a"
                    ),
                },
                "avg_expected_value": {
                    ...
                    "difference": metrics_b.get("avg_expected_value", 0)
                    - metrics_a.get("avg_expected_value", 0),
                    "winner": (
                        "b"
                        if metrics_b.get("avg_expected_value", 0)
                        > metrics_a.get("avg_expected_value", 0)
                        else "a"
                    ),
                },
                "risk_reward_ratio": {
                    ...
                    "difference": metrics_b.get("risk_reward_ratio", 0)
                    - metrics_a.get("risk_reward_ratio", 0),
                    "winner": (
                        "b"
                        if metrics_b.get("risk_reward_ratio", 0)
                        > metrics_a.get("risk_reward_ratio", 0)
                        else "a"
                    ),
                },
        ...
        score_a = metrics_a.get("avg_expected_value", 0) * metrics_a.get(
            "avg_probability_profit", 0
        )
        score_b = metrics_b.get("avg_expected_value", 0) * metrics_b.get(
            "avg_probability_profit", 0
        )
    ...
    try:
        ...
        if request.include_context:
            ...
            if recent_tests:
                ...
                performance_data = recent_tests[0].test_results.get(
                    "performance_metrics", {}
                )
        ...
        if not ai_response.get("error"):
            ...
            logger.info(
                f"AI conversation for {config_id} completed successfully but not persisted to database"
            )
...
async def get_ai_chat_history(
    config_id: str, limit: int = Query(20, description="Number of messages")
):
    """Get AI chat history for a strategy"""
    try:
        # Explicit 501 Not Implemented - prevents silent failures
        raise HTTPException(
            status_code=501, detail="AI chat history retrieval not implemented yet"
        )
    ...
    try:
        ...
        if recent_tests:
            ...
            avg_execution_time = sum(
                test.execution_time_ms for test in recent_tests
            ) / len(recent_tests)
            ...
            performance_data = {
                ...
                "latest_performance": recent_tests[0].test_results.get(
                    "performance_metrics", {}
                ),
            }
...
async def get_deployment_status(config_id: str):
    ...
    try:
        ...
        return {
            ...
            "deployed_at": (
                config.deployed_at.isoformat() if config.deployed_at else None
            ),
...
async def get_cached_market_data(symbol: str):
    ...
    try:
        # Explicit 501 Not Implemented - prevents silent failures
        raise HTTPException(
            status_code=501, detail="Cached market data retrieval not implemented yet"
        )
...
async def list_available_universes():
    ...
    try:
        ...
        return UniverseResponse(universes=universe_info, total_count=len(universe_info))
...
async def get_universe_symbols(universe_name: str):
    ...
    try:
        ...
        if universe_name not in universe_names:
            raise HTTPException(
                status_code=404, detail=f"Universe {universe_name} not found"
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded universe '{universe_name}' from file: {len(symbols)} symbols"
            )
        except Exception as e:
            ...
            logger.error(
                f"CRITICAL: Failed to load universe '{actual_universe_name}': {e}"
            )
...
async def get_user_sandbox_stats(
    user_id: str = Query("default_user", description="User identifier")
):
...
async def debug_ai_service():
    ...
    try:
        ...
        return {
            ...
            "openai_library_available": (
                ai_service.client.__class__.__name__
...
@router.get(
    "/strategies/{strategy_id}/template", response_model=StrategyTemplateResponse
)
        ...
        for json_file in strategies_dir.glob("*.json"):
            try:
                ...
                existing_strategy = next(
                    (s for s in existing_strategies if s.name == strategy_name), None
                )
                    ...
                    results.append(
                        {
                            ...
                            "universe_symbols": len(
                                sandbox_config.get("universe", {}).get(
                                    "primary_symbols", []
                                )
                            ),
                        }
                ...
                results.append(
                    {"file": json_file.name, "error": str(e), "action": "failed"}
                )
...
def convert_json_to_sandbox_config(
    json_config: Dict[str, Any], universe_loader
) -> Dict[str, Any]:
    ...
    if universe_config:
        ...
        if "universe_file" in universe_config:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to load universe from file {universe_file}: {e}"
                )
        ...
        if not symbols:
            symbols = universe_loader.get_universe("thetacrop_symbols") or [
                "SPY",
                "QQQ",
                "IWM",
            ]
